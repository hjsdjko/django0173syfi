...
<div :style='{"width":"100%","padding":"30px 18% 40px","margin":"0px auto","position":"relative","background":"#fff"}'>
    <el-form
      ...
      label-width="120px"
    >
          <el-form-item :style='{"width":"48%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="求助编号" prop="qiuzhubianhao">
            <el-input v-model="ruleForm.qiuzhubianhao" 
                placeholder="求助编号" clearable :disabled=" false  ||ro.qiuzhubianhao"></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"width":"48%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="救助回应" prop="jiuzhuhuiying">
            <el-input v-model="ruleForm.jiuzhuhuiying" 
                placeholder="救助回应" clearable :disabled=" false  ||ro.jiuzhuhuiying"></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"48%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="响应时间" prop="xiangyingshijian">
              <el-date-picker
				  :disabled=" false  ||ro.xiangyingshijian"
...
<script>
  export default {
      ...
      init(type) {
        ...
        this.$http.get(this.userTableName + '/session', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
            ...
            if((json.renyuanbianhao!=''&&json.renyuanbianhao) || json.renyuanbianhao==0){
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`jiuzhuxiangying/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('jiuzhuxiangying/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`jiuzhuxiangying/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
