...
<div :style='{"width":"100%","padding":"30px 18% 40px","margin":"0px auto","position":"relative","background":"#fff"}'>
    ...
    >
          ...
          <el-form-item :style='{"width":"48%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}'  label="慢性病" prop="manxingbing">
            <el-select v-model="ruleForm.manxingbing" placeholder="请选择慢性病" :disabled=" false  ||ro.manxingbing" multiple filterable>
              <el-option
          ...
          <el-form-item :style='{"width":"48%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="头像" v-if="type!='cross' || (type=='cross' && !ro.touxiang)" prop="touxiang">
            <file-upload
            tip="点击上传头像"
            action="file/upload"
            :limit="3"
            ...
            <el-form-item :style='{"width":"48%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' class="upload" v-else label="头像" prop="touxiang">
  ...
  export default {
    ...
    methods: {
      ...
      init(type) {
        ...
        this.$http.get('option/manxingbing/manxingbing', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
            this.manxingbingOptions = res.data.data;
          }
        });
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`laoren/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
            this.ruleForm = res.data.data;
            this.ruleForm.manxingbing = this.ruleForm.manxingbing.split(",");
          }
        });
      },
      // 提交
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('laoren/list', {
							...
							if(res.data.data.total>=crossoptnum) {
								...
								this.ruleForm.manxingbing = this.ruleForm.manxingbing.join(",");
								this.$http.post(`laoren/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
