...
<div :style='{"width":"100%","padding":"30px 18% 40px","margin":"0px auto","position":"relative","background":"#fff"}'>
    ...
    >
          <el-form-item :style='{"width":"48%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="求助编号" prop="qiuzhubianhao">
              <el-input v-model="ruleForm.qiuzhubianhao" placeholder="求助编号" disabled></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"width":"48%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="求助时间" prop="qiuzhushijian">
              <el-date-picker
          ...
          <el-form-item :style='{"width":"48%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}'  label="求助状态" prop="qiuzhuzhuangtai">
            <el-select v-model="ruleForm.qiuzhuzhuangtai" placeholder="请选择求助状态" :disabled="true  ||ro.qiuzhuzhuangtai" >
              <el-option
                  v-for="(item,index) in qiuzhuzhuangtaiOptions"
                  :key="index"
                  :label="item"
                  :value="item">
              </el-option>
          ...
          <el-form-item :style='{"width":"48%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="求助内容" prop="qiuzhuneirong">
            <editor 
...
<script>
  export default {
    data() {
      ...
      return {
        ...
        ro:{
				...
				qiuzhuneirong : false,
				qiuzhushijian : false,
				qiuzhuzhuangtai : false,
        },
        type: '',
        userTableName: localStorage.getItem('UserTableName'),
        ruleForm: {
          qiuzhubianhao: this.getUUID(),
          laorenzhanghao: '',
          laorenxingming: '',
          shoujihaoma: '',
          dizhi: '',
        ...
        rules: {
          ...
          qiuzhuneirong: [
            { required: true, message: '求助内容不能为空', trigger: 'blur' },
          ],
          qiuzhushijian: [
          ],
          qiuzhuzhuangtai: [
      ...
      init(type) {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            ...
            if(o=='qiuzhuzhuangtai'){
              this.ruleForm.qiuzhuzhuangtai = obj[o];
              this.ro.qiuzhuzhuangtai = true;
              continue;
            }
          }
        }else if(type=='edit'){
        ...
        this.$http.get(this.userTableName + '/session', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
            ...
            if((json.laorenzhanghao!=''&&json.laorenzhanghao) || json.laorenzhanghao==0){
                this.ruleForm.laorenzhanghao = json.laorenzhanghao
            }
            if((json.laorenxingming!=''&&json.laorenxingming) || json.laorenxingming==0){
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`jinjiqiuzhu/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
      ...
      onSubmit() {
			...
			if(this.ruleForm.qiuzhubianhao){
				this.ruleForm.qiuzhubianhao = String(this.ruleForm.qiuzhubianhao)
			}
			//更新跨表属性
			var crossuserid;
			var crossrefid;
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('jinjiqiuzhu/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`jinjiqiuzhu/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
